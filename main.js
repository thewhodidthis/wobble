...
// Set up with number of strips (resolution), get a lambda
// for processing each data frame in return.
export default function wobble(depth = 40) {
  // For caching consecutive frames.
  const store = []

  // Accepts and returns an `ImageData` like object, of which `data`
  // of type `Uint8ClampedArray` is the only required property.
  return (input = { data: [] }) => {
